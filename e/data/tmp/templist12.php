	...
	<div class="width960 mainContent">
		...
		<div class="content">
			...
			<div style="position:absolute;z-index:999;">
								<ul>
<?php
$cid=$GLOBALS['navclassid'];  //获得当前栏目ID
$bid = $class_r[$cid]['bclassid']; //获得父栏目ID
$lid = $class_r[$cid][islast]; //获得终极栏目值
$sqls="select classname,intro,classimg,classpath from {$dbtbpre}enewsclass where bclassid='$bid'";  //有父栏目执行此查询
if($bid==0&&$lid){
$sqls="select classname,intro,classimg,classpath from {$dbtbpre}enewsclass where classid='$cid'";
}else if($bid==0&&!$lid){
$sqls="select classname,intro,classimg,classpath from {$dbtbpre}enewsclass where bclassid='$cid'";
}else{
$sqls="select classname,intro,classimg,classpath from {$dbtbpre}enewsclass where bclassid='$bid'";
}?>
<?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq($sqls,0,24,0,0,0);
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
?>
<li>
<a class="sLink" title="<?=$bqr[classname]?>" href="<?=$public_r[newsurl]?><?=$bqr[classpath]?>">
<span class="f left"></span>
<span class="left"><?=$bqr[classname]?></span>
<span class="ta right"></span></a>
<div class="clear"></div>
</li>
<?php
}
